...
import com.raven.swing.table.ModelAction1;
...
public class Task implements Comparable<Task> {
     ...
     */
    public Task(Title title, Date startDay, Date endDay, ArrayList<People> assignments, Status status, Label labels, Major majorLabel, int time, ArrayList<Task> dependentTasks, int degree) {
        this.title = title;
        this.startDay = startDay;
        this.endDay = endDay;
        this.assignments = assignments;
     ...
     * @param unsuitable A flag indicating if the person is unsuitable for the
     * task.
     */
     ...
     *
     * @param assignments The assignments to set.
     * @param suitable A flag indicating if the assignments are suitable for the
     * task.
     */
    ...
    public Object[] toRowTable(EventAction1 event) {
        // Sử dụng DecimalFormat để định dạng ngày thành chuỗi
        DecimalFormat dateFormat = new DecimalFormat("dd/MM/yyyy");

        // Tạo đối tượng ModelAction với thông tin của đối tượng Task
        ModelAction1 modelAction1 = new ModelAction1(this, event);
